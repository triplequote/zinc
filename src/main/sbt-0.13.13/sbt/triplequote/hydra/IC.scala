package sbt.triplequote.hydra

import java.io.File
import sbt.inc.Locate._
import sbt._
import sbt.compiler._
import sbt.inc.{IncrementalCompile => _, _}
import sbt.inc.triplequote.hydra.IncrementalCompile
import xsbti.Logger
import xsbti.compile.ClasspathOptions
import xsbti.compile.CompileOrder._
import xsbti.compile.ScalaInstance
import xsbti.{ Reporter, Logger, Maybe }
import xsbti.compile.{ Output => APIOutput }
...
object IC extends IncrementalCompiler[Analysis, AnalyzingCompiler] {
  ...
  def incrementalCompile(scalac: AnalyzingCompiler,
    ...
    if (skip) Result(previousAnalysis, setup, false)
      ...
      val (analysis, changed) = compileInternal(MixedAnalyzingCompiler(config)(log))(log,
        equivCompileSetup(CompileSetup.equivOutput, hydraEquivOpts, CompileSetup.equivCompilerVersion))
  ...
  /*
    The following two implicits are copied as they are from Sbt, including strange comments. We only changed the equivalence
    relation between compiler options, as we want to ignore Hydra-specific compiler options. If we don't do that, Sbt
    triggers a "clean build".
   */

  // Equiv[CompileOrder.Value] dominates Equiv[CompileSetup]
  def equivCompileSetup(implicit equivOutput: Equiv[APIOutput], equivOpts: Equiv[CompileOptions], equivComp: Equiv[String] /*, equivOrder: Equiv[CompileOrder]*/ ): Equiv[CompileSetup] = new Equiv[CompileSetup] {
  ...
  val hydraEquivOpts: Equiv[CompileOptions] = new Equiv[CompileOptions] {

    private def nonHydraOptions(opts: Seq[String]): Seq[String] = opts match {
      case "-YpartitionFie"  +: _ +: rest => nonHydraOptions(rest)
      case "-YhydraTag"      +: _ +: rest => nonHydraOptions(rest)
      case "-YrootDirectory" +: _ +: rest => nonHydraOptions(rest)
      case "-YtimingsFile"   +: _ +: rest => nonHydraOptions(rest)
      case "-cpus"           +: _ +: rest => nonHydraOptions(rest)
      case "-YhydraStore"    +: _ +: rest => nonHydraOptions(rest)
      case head +: rest if head.startsWith("-YsourcePartitioner") => nonHydraOptions(rest)
      case head +: rest => head +: nonHydraOptions(rest)
      case Seq() => opts
    }

    /** An equivalence relation on compiler options that ignores Hydra options that shouldn't trigger recompilation. */
    def equiv(a: CompileOptions, b: CompileOptions) = {
      (nonHydraOptions(a.options) sameElements nonHydraOptions(b.options)) &&
